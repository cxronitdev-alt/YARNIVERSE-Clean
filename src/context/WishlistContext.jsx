...
export function WishlistProvider({ children }) {
  ...
  // 🚀 NAYA STATE: Ye track karega ki initial data load hua hai ya nahi
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      ...
      if (user) {
        // Data fetch karne se pehle load status false rakhein
        ...
        // Data successfully fetch ho gaya hai, ab hum save karne ke liye ready hain
  ...
  // 🔥 STEP 2: Jab bhi wishlistItems change ho, use Firestore me save karein
  useEffect(() => {
    // 🛑 SOLUTION: Yahan 'isDataLoaded' check add kiya hai.
    // Jab tak purana data load nahi hota, tab tak naya data save (overwrite) nahi hoga!
  ...
  }, [wishlistItems, currentUser, isDataLoaded]);
