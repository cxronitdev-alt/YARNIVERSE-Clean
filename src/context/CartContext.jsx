...
export function CartProvider({ children }) {
  ...
  // 🔥 Har user ke liye ek unique naam banayenge
  const cartKey = user ? `yarniverse_cart_${user.uid}` : "yarniverse_cart_guest";

  const [cartItems, setCartItems] = useState([]);
  const [openCart, setOpenCart] = useState(false);

  // ✅ Jab bhi koi User Login ya Logout karega, uska apna cart load hoga
  useEffect(() => {
    const saved = localStorage.getItem(cartKey);
    if (saved) {
      setCartItems(JSON.parse(saved));
  ...
  // ✅ Cart update hone par ussi specific user ke naam par save hoga
  useEffect(() => {
    localStorage.setItem(cartKey, JSON.stringify(cartItems));
  }, [cartItems, cartKey]);
  ...
  // 🚀 FIX: Ab ye direct nayi quantity set karega (Checkout page ke hisaab se)
  const updateQuantity = (id, newQuantity) => {
    // Agar koi 0 ya minus me le jana chahe toh use 1 par hi rok de
  ...
  const getSubtotal = () => {
    return cartItems.reduce((acc, item) => {
      const actualPrice = parsePrice(item.price);
      return acc + (actualPrice * (item.quantity || 1)); // 🚀 FIX: Quantity missing ho toh 1 maanega
    }, 0);
  };
